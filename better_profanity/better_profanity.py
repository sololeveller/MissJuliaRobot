...
class Profanity:
    ...
    def load_censor_words(self, custom_words=None, **kwargs):
        ...
        custom_words = custom_words or read_wordlist(
            self._default_wordlist_filename)
    ...
    def add_censor_words(self, custom_words):
        ...
        for w in custom_words:
            self.CENSOR_WORDSET.append(
                VaryingString(w, char_map=self.CHARS_MAPPING))
    ...
    def _populate_words_to_wordset(self, words, *, whitelist_words=None):
        ...
        for word in set(words):
            ...
            all_censor_words.append(
                VaryingString(word, char_map=self.CHARS_MAPPING))
    ...
    def _update_next_words_indices(self, text, words_indices, start_idx):
        ...
        if not words_indices:
            words_indices = self._get_next_words(text, start_idx,
                                                 self.MAX_NUMBER_COMBINATIONS)
        else:
            del words_indices[:2]
            if words_indices and words_indices[-1][0] != "":
                words_indices += self._get_next_words(text,
                                                      words_indices[-1][1], 1)
    ...
    def _get_next_words(self, text, start_idx, num_of_next_words=1):
        ...
        start_idx_of_next_word = self._get_start_index_of_next_word(
            text, start_idx)
        ...
        words = [
            (next_word, end_index),
            ("%s%s" % (text[start_idx:start_idx_of_next_word], next_word),
             end_index),
        ]
        if num_of_next_words > 1:
            words.extend(
                self._get_next_words(text, end_index, num_of_next_words - 1))
